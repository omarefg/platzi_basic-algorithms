class BubbleSort {
    constructor(sequence) {
        ...
        this.lastIndex = sequence.length - 1
    }

    _swap (n1, n2) {
        const tmp = this.sequence[n1]
        this.sequence[n1] = this.sequence[n2]
        this.sequence[n2] = tmp
    }

    sort () {
        for (let i = 0; i < this.lastIndex; i++) {
            for (let j = 0; j < this.lastIndex - i; j++) {
                if (this.sequence[j] > this.sequence[j + 1]) {
                    this._swap(j, j + 1)
                }
            }
        }
