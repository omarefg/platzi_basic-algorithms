class InsertionSort:
    ...
    def sort(self):
        for i in range(self.length):
            currentVal = self.sequence[i]
            j = i - 1
            while j >= 0 and self.sequence[j] > currentVal:
                self.sequence[j + 1] = self.sequence[j]
                j = j - 1
            self.sequence[j + 1] = currentVal
...
def run():
    ...
    insertionSort = InsertionSort(sequence)
    insertionSort.sort()
    insertionSort.print()
