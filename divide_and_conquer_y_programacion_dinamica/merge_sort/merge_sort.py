class MergeSort:
    ...
    def _merge(self, low, mid, high):
        ...
        for i in range(n1):
            arr1.append(self.sequence[low + i])
        for j in range(n2):
            arr2.append(self.sequence[mid + j + 1])
        ...
        for k in range(high - 1):
            ...
            if arr1[i] <= arr2[j]:
                print(i)
                self.sequence[k] = arr1[i]
                i+=1
            else:
                self.sequence[k] = arr2[j]
                j+=1
...
def run():
    ...
    sequence = [-796, 3229, -5164, -362, 4408]
    mergeSort = MergeSort(sequence)
    mergeSort.sort(0, len(sequence) - 1)
    mergeSort.print()
