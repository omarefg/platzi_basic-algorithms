class MergeSort {
    ...
    _merge (low, mid, high) {
        ...
        for (i = 0; i < n1; i++) {
            arr1[i] = this.sequence[low + i];
        }
        for (j = 0; j < n2; j++) {
            arr2[j] = this.sequence[mid + j + 1];
        }
        ...
        for (let k = low; k <= high; k++) {
            if (arr1[i] <= arr2[j]) {
                this.sequence[k] = arr1[i++];
            } else {
                this.sequence[k] = arr2[j++];
            }
        }
    }
...
const mergeSort = new MergeSort(sequence)
mergeSort.sort(0, sequence.length - 1)
mergeSort.print()